...
private:
	...
	unsigned int Find_first_cell_number_x(Particle A, Data d) //all the private functions below are needed to calculate the number of the cell with given coords
	{
		if (d.Radius_of_Interaction > A.GetX()) { return 0; }
		return (unsigned int)(A.GetX() - d.Radius_of_Interaction) / size_of_cell.x;
	}
	unsigned int Find_first_cell_number_y(Particle A, Data d)
	{
		if (d.Radius_of_Interaction > A.GetY()) { return 0; }
		return (unsigned int)(A.GetY() - d.Radius_of_Interaction) / size_of_cell.y;
	}
	unsigned int Find_last_cell_number_x(Particle A, Data d)
	{
		if (d.Radius_of_Interaction + A.GetX() > d.boundX) { return number_of_cells.x; }
		return (unsigned int)(A.GetX() + d.Radius_of_Interaction) / size_of_cell.x;
	}
	unsigned int Find_last_cell_number_y(Particle A, Data d)
	{
		if (d.Radius_of_Interaction + A.GetY() > d.boundY) { return number_of_cells.y; }
		return (unsigned int)(A.GetY() + d.Radius_of_Interaction) / size_of_cell.y;
	}
};
